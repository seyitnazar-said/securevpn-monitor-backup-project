...
print_color "green" "Installing OpenVPN..."
...
print_color "green" "Copying server.conf file"
sudo cp "$server_conf" "$openvpn"

print_color "green" "Making changes in server.conf file"
sudo sed -i 's/server.crt/openvpn.crt/g'  /etc/openvpn/server.conf
sudo sed -i 's/server.key/openvpn.key/g' /etc/openvpn/server.conf
sudo sed -i 's/dh2048.pem/dh.pem/g' /etc/openvpn/server.conf
sudo sed -i 's/^;\s*push "route 192\.168\.10\.0 255\.255\.255\.0"/push "route 192.168.1.0 255.255.255.0"/' /etc/openvpn/server.conf
sudo sed -i 's/^tls-auth/tls-crypt/' /etc/openvpn/server.conf
sudo sed -i '/^;[[:space:]]*log/s/^;[[:space:]]*//' /etc/openvpn/server.conf
sudo sed -i -e '/^;*push "redirect-gateway def1 bypass-dhcp"/s/^;*//' /etc/openvpn/server.conf
sudo sed -i -e '$a\push "dhcp-option DNS 8.8.8.8"' /etc/openvpn/server.conf
sudo sed -i -e '$a\push "dhcp-option DNS 1.1.1.1"' /etc/openvpn/server.conf
sudo sed -i -e '$a\route-gateway 10.8.0.1' /etc/openvpn/server.conf
